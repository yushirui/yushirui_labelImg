...
class Yu(QMainWindow, WindowMixin):
    ...
    def __init__(self):
        ...
        defaultFilename = sys.argv[1] if len(sys.argv) >= 2 else None
        defaultPrefdefClassFile = sys.argv[2] if len(sys.argv) >= 3 else os.path.join(
            os.path.dirname(sys.argv[0]),
            'data', 'predefined_classes.txt')
        defaultSaveDir = sys.argv[3] if len(sys.argv) >= 4 else None
        ...
        # self.defaultLabelTextLine.setText('tongue')
        # 现在改成下拉选
        self.defaultLabelTextLine = QComboBox()
        self.defaultLabelTextLine.addItems(['tongue','eye','mouth','face'])
        self.defaultLabelTextLine.setEditable(False)
        ...
        # 修改
        self.d_auto_w = QCheckBox('d后自动w')
        self.d_auto_w.setChecked(True)
        listLayout.addWidget(self.d_auto_w)
        ...
        # 修改
        # 画布绑定双击事件，连接信号
        self.canvas.yushiruiSignStr.connect(self.yushiruiMouseClick)
        ...
        # 修改
        openNextImg = action('下一个d', self.openNextImg,
                             'd', 'next', u'Open Next')

        openPrevImg = action('上一个a', self.openPrevImg,
                             'a', 'prev', u'Open Prev')

        verify = action('&Verify Image', self.verifyImg,
                        'space', 'verify', u'Verify Image')

        save = action('保存s', self.saveFile,
        ...
        # 修改
        createMode = action('框框w', self.setCreateMode,
                            'w', 'new', u'Start drawing Boxs', enabled=False)
        editMode = action('&Edit\nRectBox', self.setEditMode,
                          'Ctrl+J', 'edit', u'Move and edit Boxs', enabled=False)

        create = action('框框w', self.createShape,
        ...
        # 修改
        help = action('百度一下', self.showTutorialDialog, 'F2', 'help', '百度一下')

        # 修改（动作栏标题，方法打开信息框，快捷键F1/None，挂在help下，窗口标题）
        showInfo = action('余时锐使用帮助', self.showInfoDialog, 'F1', 'help', '余时锐使用帮助')
    ...
    # 修改，鼠标点击事件槽函数
    def yushiruiMouseClick(self, text):
        try:
            if text == '双击左键画框框':
                # 画框框
                self.createShape()
            ...
            print('忽略，main.yushiruiMouseClick错误')
...
'''
        QMessageBox.information(self, '余时锐使用帮助', msg)
    ...
    def newShape(self):
        ...
        """
        if not self.useDefaultLabelCheckbox.isChecked() or not self.defaultLabelTextLine.currentText():
            ...
            text = self.defaultLabelTextLine.currentText()
    ...
    def resizeEvent(self, event):
        if self.canvas and not self.image.isNull() \
            ...
            self.adjustScale()
        super(Yu, self).resizeEvent(event)
    ...
    def openNextImg(self, _value=False):
        ...
        # 修改
        # 如果【d后加w】勾选
        if self.d_auto_w.isChecked():
            # 创建框框
            self.createShape()
            
        # 标记脸
        if self.defaultLabelTextLine.currentText() == 'face':
            # 放大1倍
            self.addZoom(100)
        # 标记眼睛
        elif self.defaultLabelTextLine.currentText() == 'eye':
            # 放大5倍
            self.addZoom(500)
            # 改变拖动条位置，垂直1/4
            self.scrollBars[Qt.Vertical].setValue(self.canvas.height() // 6)
        # 标记嘴巴、舌头
        elif self.defaultLabelTextLine.currentText() == 'mouth' or self.defaultLabelTextLine.currentText() == 'tongue':
            # 放大5倍
            self.addZoom(500)
            # 改变拖动条位置，垂直1/2
            self.scrollBars[Qt.Vertical].setValue(self.canvas.height() // 4)
        # 改变拖动条位置，水平1/3
        self.scrollBars[Qt.Horizontal].setValue(self.canvas.width() // 3)
    ...
    def saveFileDialog(self):
        ...
        # 当前文件，绝对路径，C:\Users\yushirui\Desktop\余时锐20200817\要标注\yushirui000001
